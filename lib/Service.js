...
class Service extends EndpointsMixin(stm.StateTransitionMixin(llm.LogLevelMixin(_Service, llm.defaultLogLevels,
	...
	/**
	 * Meta infogmation for the config attributes.
	 * - default optional default value of the attribute
	 * - needsRestart optional modification requres a service restart
	 ...
	 * @return {Object} attribute meta information
	 */
	get configurationAttributes() {
		return {
			logLevel: {
				default: llm.defaultLogLevels.info,
				setter(newValue, attribute) {
					if (newValue) {
	...
	/**
	 * Takes attribute values from config parmeter
	 * and copies them over to the target object.
	 * Copying is done according to configurationAttributes
	 * @param {Object} config
	 * @return {Set} of modified attributes
	 */
	_configure(config) {
		const cas = this.configurationAttributes;
		const modified = new Set();
		Object.keys(cas).forEach(name => {
			const ca = cas[name];
			const value = config[name] || ca.default;
			if (ca.setter) {
				if (ca.setter.call(this, value, ca)) {
					modified.add(ca);
				}
			} else {
				if (this[name] !== value && value !== undefined) {
					this[name] = value;
					modified.add(ca);
				}
			}
	 ...
	 * Internally calls _configure(config) as the constuctor does
	 * If attribute with nnnedsRestart are touched the restartIfRunning will be called
