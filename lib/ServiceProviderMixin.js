	...
	constructor(config, owner) {
		...
		function execCommand(sp, command) {
			if (command.action === 'list') {
				return Promise.resolve(Object.keys(sp.services).map(name => sp.services[name]).forEach(s => s.service.toJSONWithOptions(
					command.options)));
			}

			const service = sp.services[command.service];
