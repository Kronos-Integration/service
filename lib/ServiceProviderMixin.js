...
module.exports = (superclass) => class extends superclass {
	...
	declareService(config, waitUntilFactoryPresent) {
		...
		//console.log(`[${seq}] ENTRY ${JSON.stringify(config)}`);
		...
		if (service === undefined) {
			...
			if (this._declareServicePromises) {
				...
				//console.log(`[${seq}] PROMISE ${name} -> ${p ? 'promise ongoing' : 'new promise'}`);
			...
			const insertIntoDeclarePromisesAndDeliver = () => {
				const p = this.registerService(this.createServiceFactoryInstanceFromConfig(config, this)).then(
					s => {
						...
						//console.log(`[${seq}] REMOVE PROMISE ${name} deliver: ${s}`);
						return Promise.resolve(s);
					});

				this._declareServicePromises.set(name, p);

				//console.log(`[${seq}] PRESERVE PROMISE ${name}`);
			...
			if (waitUntilFactoryPresent && !this.serviceFactories[type]) {
				return new Promise((fullfill, reject) => {
					const listener = factory => {
						//console.log(`[${seq}] serviceFactoryRegistered: ${factory.name} <> ${type}`);
		...
		//console.log(`[${seq}] DIRECT ${name} deliver: ${service}`);
